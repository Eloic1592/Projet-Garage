...
public class ListeVehiculeServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(request.getParameter("client") != null) {
            ...
            RequestDispatcher dispatch = request.getRequestDispatcher("liste-vehicule.jsp");
            try {    
                Liste_vehicule serviceVehicule = new Liste_vehicule();
                serviceVehicule.setIdclient(client);
                String[] colonne = new String[1];
                colonne[0] = "idclient";

                Vector<Liste_vehicule> vehicules = serviceVehicule.getBy(null, colonne);
                request.setAttribute("vehicules", vehicules);
            } catch (SQLException e1) {
                request.setAttribute("sqlError", 1);
                // e1.printStackTrace(response.getWriter());
            } catch (Exception e2) {
                request.setAttribute("globalError", 1);
            ...
            RequestDispatcher dispatch = request.getRequestDispatcher("liste-client.jsp");
            dispatch.forward(request, response);
        }
    }
